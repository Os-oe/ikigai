...
(function () {
  ...
  window.IKIGAI_VENN = function (data) {
    ...
    svg += '<style>' +
      '.vc{fill-opacity:.16;stroke-opacity:.55;stroke-width:1.5}' +
      '.v-label{font:600 13px Inter,sans-serif;letter-spacing:.08em;text-transform:uppercase}' +
      '.v-term{font:italic 500 15px "Shippori Mincho",Georgia,serif}' +
      '.v-overlap{font:italic 500 12.5px "Shippori Mincho",Georgia,serif;fill:#5b5347}' +
      '.v-zentrum{font:600 17px "Shippori Mincho",Georgia,serif;fill:#211d18}' +
      '.v-ikigai{font:600 11px Inter,sans-serif;fill:#f6303a;letter-spacing:.18em}' +
      '@media(max-width:560px){.v-term{font-size:16px}}' +
      '</style>';
    ...
    GEO.circles.forEach(function (ci) {
      ...
      var lh = 20, startY = ly - ((lines.length - 1) * lh) / 2;
    ...
    var zlh = 22, zy = c - ((zl.length - 1) * zlh) / 2 - 6;
    svg += '<text class="v-ikigai venn-t" text-anchor="middle" x="' + c + '" y="' + (zy - 28) + '">IKIGAI</text>';
    svg += '<text text-anchor="middle" class="venn-t">';
