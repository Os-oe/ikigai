// lz-string 1.4.4 (pieroxy) — URL-safe subset: compressToEncodedURIComponent /
// decompressFromEncodedURIComponent. MIT. Für den ikigAI-Permalink (#r=…) —
// Ergebnis komprimiert ins URL-Fragment, geht nie zum Server (Datenschutz bleibt wahr).
...
var LZString={compressToEncodedURIComponent:function(input){if(input==null)return"";return LZString._compress(input,6,function(a){return keyStrUriSafe.charAt(a);});},decompressFromEncodedURIComponent:function(input){if(input==null)return"";if(input=="")return null;input=input.replace(/ /g,"+");return LZString._decompress(input.length,32,function(index){return getBaseValue(keyStrUriSafe,input.charAt(index));});},
